...
async function signTruncateCore(core, signatures, length, { commit } = {}) {
  /** @type {Hypercore | null} */
  let batch = null
  try {
    batch = await createTruncateBatch(core, length)
    console.log('🚀 ~ signTruncateCore ~ batch:', batch.fork, batch.length)
    ...
    if (commit) {
      await core.commit(batch, { signature, length })
      console.log('🚀 ~ signTruncateCore ~ core:', core.fork, core.length)
    }
