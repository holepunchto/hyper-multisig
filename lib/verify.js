...
async function verifyCoreCommittable(
  ...
  if (!b4a.equals(await srcCore.treeHash(length), treeHash)) {
    throw MultisigError.SRC_KEY_MISMATCH({ coreId })
  }
  ...
  if (!b4a.equals(await Hypercore.treeHashFromStorage(tgtCore), await Hypercore.treeHashFromStorage(srcCore, tgtCore.length))) {
    throw MultisigError.INCOMPATIBLE_SOURCE_AND_TARGET({ coreId })
  }

  if (!skipTargetWellSeeded) {
    let tgtFullCopies = 0
