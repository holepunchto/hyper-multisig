 ...
 * @return {Promise<{ key: string, keyHex: string, length: number, treeHash: string, treeHashHex: string }>}
 */
async function getCoreInfo(core) {
  ...
  return {
    ...
    length: core.length,
    treeHash: idEnc.normalize(treeHash),
    treeHashHex: treeHash.toString('hex')
  }
}
