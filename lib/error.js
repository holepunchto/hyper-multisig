class MultisigError extends Error {
  ...
  static REQUEST_LENGTH_TOO_SMALL(length, { coreId = '' } = {}) {
    return new MultisigError(
      `${coreId} Request length of ${length} is smaller than the target core length`,
      'REQUEST_LENGTH_TOO_SMALL',
      MultisigError.REQUEST_LENGTH_TOO_SMALL
    )
  }
  ...
  static SOURCE_CORE_TREE_HASH_MISMATCH(length, { coreId = '' } = {}) {
    return new MultisigError(
      `${coreId} Source core tree hash at length ${length} does not match the expected value`,
      'SOURCE_CORE_TREE_HASH_MISMATCH',
      MultisigError.SOURCE_CORE_TREE_HASH_MISMATCH
    )
  }
