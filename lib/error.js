class MultisigError extends Error {
  ...
  static COMMIT_FAILED() {
    return new MultisigError(
      'Failed to commit the batch',
      'COMMIT_FAILED',
      MultisigError.COMMIT_FAILED
    )
  }

  static COMMITTED_LENGTH_MISMATCH() {
    return new MultisigError(
      'The committed length does not match the expected length',
      'COMMITTED_LENGTH_MISMATCH',
      MultisigError.COMMITTED_LENGTH_MISMATCH
    )
  }
}
