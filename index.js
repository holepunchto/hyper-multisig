...
class HyperMultisig {
  ...
  requestCore(
    ...
    srcCore,
    length,
    { force = false, quorum, peerUpdateTimeout, version } = {}
  ) {
    return new HyperMultisigRunner(async (runner) => {
      ...
      const manifest = getManifest(publicKeys, namespace, { quorum })
      const request = await SignRequest.generate(srcCore, { manifest, length, version })
  ...
  requestDrive(
    ...
    return new HyperMultisigRunner(async (runner) => {
      ...
      const manifest = getManifest(publicKeys, namespace, { quorum })
      const request = await SignRequest.generateDrive(srcDrive, { manifest, length, version })
      return { manifest, request }
    })
  }
