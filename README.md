...
import HyperMultisig from 'hyper-multisig'
...
Create a multisig core from a set of signer public keys:

```js
// collect public keys of all signers generated by hypercore-sign
const publicKeys = ['o37a1...', 'qgbd9...', '6r66j...']

// to avoid collision, structure namespace based on repo path
...
await srcCore.append('hello world 1')
await srcCore.append('hello world 2')
await srcCore.append('hello world 3')

const { request } = await multisig.requestCore(publicKeys, namespace, srcCore, 3).done()
```

Sign the request externally using [hypercore-sign](https://github.com/holepunchto/hypercore-sign):

```sh
hypercore-sign <signingRequest>
```

Commit the multisig core with the collected signatures:
